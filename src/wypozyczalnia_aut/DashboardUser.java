...
public class DashboardUser extends javax.swing.JFrame {
    ...
    public DashboardUser() {
        ...
        getUserSummaryStats();
        ClientLabel.setText(controller.getUserData(controller.userId, "username")[0]);
    }
    ...
    public static void getUserSummaryStats() {
        Map<String, Object> carData = null;
        try {
            carData = controller.getUserSummary(Controller.userId);

            java.sql.Date nearestReturnDate = (java.sql.Date) carData.get("nearestReturnDate");
            if (nearestReturnDate != null) {
                LocalDate localDate = nearestReturnDate.toLocalDate();
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd.MM.yyyy");
                String formattedDate = localDate.format(formatter);

                totalCarsLabel.setText(carData.get("numReservations").toString());
                nearestReturnDateLabel.setText(formattedDate);
                totalCostsLabel.setText(carData.get("totalCost").toString());
            }
        } catch (Throwable t) {
            t.printStackTrace();
        }
